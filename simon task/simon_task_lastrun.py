...
# Run 'Before Experiment' code from simon_config
left_stim_text = ''  
right_stim_text = ''
print(f"Initial values - left_stim_text: {left_stim_text}, right_stim_text: {right_stim_text}")
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Hansika Garg\\OneDrive\\Desktop\\simon_task_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    trials = data.TrialHandler(nReps=2.0, method='sequential', 
        ...
        trialList=data.importConditions('simon-task_order.xlsx'),
    ...
    for thisTrial in trials:
        ...
        thisExp.addData('simon_setup.started', globalClock.getTime(format='float'))
        ...
        # Trial loop
        for thisTrial in trials:
            # Debugging: Verify thisTrial content
            print(f"Current trial: {thisTrial}")
        
            # Check if keys exist in thisTrial
            if 'conditions' in thisTrial and 'stim' in thisTrial:
                if expInfo['counterbalance'] == 'O':
                    if thisTrial['conditions'] == 'incongruent':
                        if thisTrial['stim'] == 'X':
                            ...
                            ans = 'z'
                    elif thisTrial['conditions'] == 'congruent':
                        if thisTrial['stim'] == 'X':
                            left_stim_text = thisTrial['stim']
                            right_stim_text = ''
                            ans = 'm'
                ...
                print("Error: Missing 'conditions' or 'stim' in thisTrial.")
                left_stim_text = ''
                right_stim_text = ''
        
            # Debugging: Print values before using them
            print(f"left_stim_text: {left_stim_text}, right_stim_text: {right_stim_text}, ans: {ans}")
        ...
        thisExp.addData('simon_setup.stopped', globalClock.getTime(format='float'))
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('simon_trial.started', globalClock.getTime(format='float'))
        left_stim.setText(left_stim_text)
        right_stim.setText(right_stim_text)
        ...
        for thisComponent in simon_trialComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('simon_trial.stopped', globalClock.getTime(format='float'))
        # check responses
